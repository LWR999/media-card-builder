...
def start_build(card_id: int, db_params: dict, nas_root: str, stage_path: str) -> bool:
    with _lock:
    ...
    threading.Thread(
        target=_run_build,
        args=(card_id, db_params, nas_root, stage_path),
        daemon=True,
    ).start()
    return True
...
def _run_build(card_id: int, db_params: dict, nas_root: str, stage_path: str):
    ...
    try:
        ...
        for idx, (album_id, artist, title, nas_path) in enumerate(rows):
